...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    const fetchBookedTimes = async () => {
      try {
        setBookedTimesStatus((prev) => ({
          ...prev,
          [selectedDate]: { loading: true, error: false },
        }));

        const res = await fetch(`http://localhost:5000/api/bookings?date=${selectedDate}`, { signal: controller.signal });
        ...
        if (!res.ok) {
          if (res.status === 404) {
            ...
            setBookedTimesStatus((prev) => ({
              ...prev,
              [selectedDate]: { loading: false, error: false },
            }));
            return;
          }
        ...
        const times = Array.isArray(data) ? data.map((b) => b?.time).filter(Boolean) : [];
        ...
        setBookedTimesStatus((prev) => ({
          ...prev,
          [selectedDate]: { loading: false, error: true },
        }));
      }
    };
  ...
  }, [selectedDate, timeSlots.length, fullyBookedDates
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setBookedTimesByDate((prev) => {
        const prevTimes = prev[selectedDate] || [];
        const nextTimes = prevTimes.includes(form.time)
          ? prevTimes
          : [...prevTimes, form.time];
        return { ...prev, [selectedDate]: nextTimes };
      });

      // ✅ check if fully booked now
      setFullyBookedDates((prev) => {
        const allTimes = bookedTimesByDate[selectedDate] || [];
        const newTimes = allTimes.includes(form.time)
          ? allTimes
          : [...allTimes, form.time];
        if (newTimes.length >= timeSlots.length && !prev.includes(selectedDate)) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4 bg-gray-300 rounded shadow p-4 sm:p-6">
            ...
            <div className="mt-4 p-3 sm:p-4 border border-gray-100 rounded w-full max-w-sm mx-auto bg-gray-400">
              <div className="flex justify-between items-center mb-2">
                <button type="button" onClick={() => setCalendarMonth(new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() - 1, 1))} className="px-2 py-1 rounded hover:bg-gray-100">←</button>
            ...
            <div className="mt-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 w-full max-w-sm mx-auto">
                {timeSlots.map((t) => {
                  const bookedTimesForDate = bookedTimesByDate[selectedDate] || [];
                  const status = bookedTimesStatus[selectedDate] || { loading: false, error: false };

                  const isBooked = bookedTimesForDate.includes(t);
                  const disableTime = !selectedDate || status.loading || isBooked || isTimePast(selectedDate, t);
              ...
              {bookedTimesStatus[selectedDate]?.loading && (
                <div className="text-gray-600 text-sm text-center mt-2">Tijden laden...</div>
              )}
              {bookedTimesStatus[selectedDate]?.error && (
                <div className="text-yellow-600 text-sm text-center mt-2">
                  Kon geboekte tijden niet ophalen — probeer opnieuw.
                </div>
              )}
            </div>
            ...
            <div className="flex justify-center mt-6">
              <button
                ...
                className={`px-6 py-3 rounded text-sm sm:text-base ${
                  !submitting ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-blue-300 text-white cursor-not-allowed opacity-70"
                }`}
              >
