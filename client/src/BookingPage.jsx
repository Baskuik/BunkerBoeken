...
export default function BookingPage() {
  ...
  // Laad teksten + openingHours van backend
  useEffect(() => {
    ...
    fetch("http://127.0.0.1:5000/api/settings/editableText")
      .then(res => res.json())
      .then(data => {
        if (data.value) setEditableText(data.value);
      })
  ...
  // Sla editable texts op bij Done
  const saveEditableText = async () => {
    try {
      await fetch("http://127.0.0.1:5000/api/settings/editableText", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ value: editableText }),
      });
      alert("Teksten opgeslagen!");
    } catch (err) {
      console.error("Kon teksten niet opslaan:", err);
      alert("Fout bij opslaan teksten");
    }
  };

  const toggleEditing = () => {
    if (isEditing) saveEditableText();
